...
/*
 ...
 >> JDBC Driver가 포함된 라이브러리 파일(Jar 파일)를 다운로드 받아 프로젝트 빌드 처리해야만 라이브러리의 클래스를 사용하여 JDBC 프로그램 작성 가능
 ...
 3. 프로젝트 폴더에 저장된 라이브러리 파일을 프로젝트에서 사용할 수 있도록 연결 - 빌드 처리
 	>>라이브러리 파일(Jar)의 클래스 및 인터페이스를 프로젝트에서 사용 가능하도록 설정
  	>>프로젝트 > 마우스 오른쪽 > Properties > Java Build Path(메뉴) > Libraries(탭) 클릭 > classpath(선택) > Add Jars(메뉴) > 프로젝트의 Jar 파일 선택 > Apply and Close
*/
...
public class InsertStudentApp {
	public static void main(String[] args) {
		//JDBC 관련 객체를 저장하기 위한 참조변수는 try 영역 외부에서 선언 > try 영역을 포함한 모든 영역에서 참조변수를 이용하여 객체를 사용 가능
		Connection con =null;
		Statement stmt =null;
		...
		/*
		 1. Oracle Driver 클래스로 객체를 생성하여 DriverManager 클래스의 JDBC Driver 객체로 등록
		 	>>Oracle Driver 클래스를 읽어 메모리에 저장하여 자동으로 Oracle Driver 객체를 생성하여 DriverManager클래스의 JDBC Driver 객체 등록
		 ...
		 DriverManager.registerDriver(Driver driver): Driver 객체를 매개변수로 전달받아 DriverManager클래스가 관리할 수 있는 Driver 객체를 등록하는 메소드
		 	>> 동일한 클래스로 생성된 Driver 객체가 DriverManager 클래스에 여러개 등록 가능
		 	>> 불필요한 Driver 객체가 존재하여 성능의 저하 발생 > 객체 직접 만드는것을 권장하지 않음 
		DriverManager.registerDriver(new OracleDriver());
		 */
		//string className 메소드를 호출하여 ClassLoader 프로그램을 이용하여 OracleDriver 클래스를 읽어 메모리에 저장
		// >> OracleDriver클래스의 정적영역에서 OracleDriver클래스를 객체로 생성하여 DriverManager클래스의 JDBC Driver로 등록하는 메소드 호출
			
		Class.forName("oracle.jdbc.driver.OracleDriver");// > 접속 환경 생성  > 서버이름, 포트, 계정명, 비밀번호 등 제공 해줘야함
		...
		/*
		 ...
		 DriverManager.getConnection(String url, String user, String password)
		  ...
		  >> Connection 객체를 반환받아 저장 > DBMS 서버에 접속된 정보가 저장되어 있음
		 ...
		 */
			...
			con = DriverManager.getConnection(url, user, password);// > 접속 안끊으면 계속 컨넥션이 유지되어있음 > 끊어줘야함 그래야 컨넥션 다시 만들어 쓸 수 있음 - try에서 끊으면 안됨 예외가 있던없던 끊어줘여ㅑ함
		...
		/*
		 4. Statement 객체를 사용하여 SQL 명령(DML 명령 : INSERT, DELETE, SELECT, UPDATE))을 DBMS 서버에 전달하여 실행하고 실행결과를 반환받아 저장
		 ...
		 */
			...
			///String sql = "insert into student values(1000,'홍길동','010-1111-1111','서울시 강남구','00/01/01')";
			//String sql = "insert into student values(2000,'임꺽정','010-2222-2222','수원시 월정구','00/01/02')";
			String sql = "insert into student values(3000,'전우치','010-3333-3333','인천시 상당구','00/01/03')";
		 ...
		 //5. 반환받은 SQL 명령의 실행 결과값을 사용자에게 제공 - 출력
			...
			try {
				...
				e.printStackTrace();
			}//try에서 생성된 변수는 try안에서만 사용 가능 - 에러
		}
	}
}

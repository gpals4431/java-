...
//UCP(Universal Connection Pool) 라이브러리에 의해 제공되는 DBCP 클래스를 이용한 JDBC 프로그램 작성
// >> http://www.oracle.com 사이트의 페이지에서 UCP 라이브러리 파일(ucp11.jar)을 다운로드 받아 프로젝트에 빌드 처리 

public class DataSourceApp {
	public static void main(String[] args) throws SQLException {
		//PoolDataSource 객체(dbcp 객체)를 반환받아 저장 : PoolDataSource 객체를 생성하여 반환하는 메소드
		// new 보단 factory 클래스 사용하는것이 효율적임
		...
		//PoolDataSourceFactory.setURL(String url)
		// >> PoolDataSource 객체의 사용자 이름를 변경하는 메소드 
		pds.setUser("scott");
		
		//PoolDataSourceFactory.setURL(String url)
		// >> PoolDataSource 객체의 비밀번호 를 변경하는 메소드 

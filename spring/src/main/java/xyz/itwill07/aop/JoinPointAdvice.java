...
public class JoinPointAdvice {
	...
	//AfterReturning Advice Method에는 JoinPoint 인터페이스의 매개변수 외에 Object 클래스의 매개변수를 선언할 수 있음
	//=> 스프링 컨테이너는 Object 클래스의 매개변수에 타겟메소드의 반환값을 제공받아 전달
	//=> 타겟메소드에 반환되는 값(객체)의 자료형이 고정되어 있는 경우 Object 클래스 대신 
	//반환되는 값 또는 객체에 자료형으로 매개변수를 작성 가능
	//=> Object 클래스의 매개변수가 선언된 경우 Spring Bean Configuration File의 AOP 설정에서
	//after-returning 엘리먼트에 반드시 returning 속성을 사용하여 반환값을 저장할 매개변수의 이름을 속성값으로 지정
	...
	public void displayName(JoinPoint joinPoint, Object object) {
		//out.println("### [after-Returning] 핵심관심코드가 정상적으로 실행 후 삽입되어 실행될 횡단관심코드 ###");
		
		String methodName=joinPoint.getSignature().getName();
		String userName=(String)object;
		System.out.println("### [before]"+methodName+" 메소드의 반환값 = "+userName);
		
	}
	/*AfterReturning Advice Method에는 JoinPoint 인터페이스의 매개변수 외에 Exception 클래스의 매개변수 선언가능
	 => 스프링 컨테이너는 Object 클래스의 매개변수에 타겟메소드에서 발생된 예외 객체를 제공받아 저장
	 ...
	 after-Throwing 엘리먼트에 반드시 Throwing 속성을 사용하여 발생된 예외를 저장하기위한 매개변수의 이름을 속성값으로 지정
	 => after-Throwing 엘리먼트에 Throwing 속성이 없거나 속성값이 잘못된 경우 IllegalArgumentException 발생
	 */
	
	//AfterThrowing Advice Method
	public void exceptionHandle(JoinPoint joinPoint, Exception exception) {
		//System.out.println("### [after-Throwing] 핵심관심코드가 정상적으로 실행 후 삽입되어 실행될 횡단관심코드 ###");
		
		String methodName=joinPoint.getSignature().getName();
		System.out.println("### [before]"+methodName+" 메소드에서 발생된 예외 = "+exception.getMessage());

	}
	/*Around Advice Method는 반환형을 Object 클래스로 작성하고 매개변수는 ProceedingJoinPoint 인터페이스로 작성
	=> 타겟 메소드의 반환값을 제공받아 반환처리하기 위해 Object 클래스로 반환형 작성
	=> 타겟 메소드 관련 정보를 ProceedingJoinPoint 인터페이스의 매개변수로 제공받아 
	...
	public Object display(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		//proceed() : 타겟 메소드를 호출하는 메소드 - 핵심관심코드 실행
		//=> 타겟메소드를 호출하여 반환되는 결과값을 변수에 저장 - 반환처리
		//=> Throwable(Error 클래스와 Exception 클래스의 부모클래스 - 모든 오류)객체가 발생되므로 
		//반드시 예외처리하거나 예외 전달

...
public class AopAnnotationAdvice {
	...
	//@Before : 타겟메소드의 핵심관심코드 실행 후에 예외에 상관없이 무조건 실행될 횡단관심코드를 제공하기위한 어노테이션
	@After("aopPointCut()")
	public void afterLog() {
		log.info("[after] 핵심관심코드 실행 후에 예외에 상관없이 무조건 실행될 횡단관심코드");	
	}
	
	//=>returning 속성: 타겟메소드의 반환값을 전달받아 저장하기 위한 매개변수의 이름을 속성값을 설정
	@AfterReturning(value="aopPointCut()", returning ="object")
	public void afterReturningLog(Object object) {
		log.info("[afterReturning] 핵심관심코드 정상적으로 실행 후에 실행될 횡단관심코드");	
	...
	public void afterThrowingLog() {
		log.info("[afterThrowing] 핵심관심코드 실행시 예외가 발생된 후 실행될 횡단관심코드");	
	...
	public Object aroundLog(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		log.info("[after] 핵심관심코드 실행 후 삽입되어 실행될 횡단관심코드");	
		return result;
	}
}

...
//횡단관심모듈(CrossCutting Consern Module) : 횡단관심코드로 작성된 메소드가 선언된 클래스 -
//=> 횡단관심코드 : 데이터 처리 명령외에 보조적인 기능을 제공하는 명령
//=> 로그처리, 보안, 권한처리, 트렌젝션 처리, 예외 처리 등의 명령
public class HewonAdvice {
	...
	타겟메소드(Target Method) : 핵심관심 모듈의 메소드 중 pointcut 표현식으로 지정되어 횡단관심코드에 삽입될 메소드
	=>pointcut 표현식 : 핵심관심모듈의 메소드 중 특정 메소드만 지정하기 위한 언어*/
	public void beforeLog() {
		System.out.println("### [before] 핵심관심코드 실행 전 삽입되어 실행될 횡단관심코드 ###");
	}
	
	/*타겟 메소드의 명령 실행 후 무조건 삽입되어 실행될 명령을 작성한 메소드 - After Advice Method*/
	public void afterLog() {
		System.out.println("### [after] 핵심관심코드 실행 후 무조건 삽입되어 실행될 횡단관심코드 ###");
	}
	
	/*타겟 메소드의 명령이 정상적으로 실행 후 무조건 삽입되어 실행될 명령을 작성한 메소드 - After Returning Method*/
	public void afterReturningLog() {
		System.out.println("### [after-Returning] 핵심관심코드가 정상적으로 실행 후 삽입되어 실행될 횡단관심코드 ###");
		
	}
	/*타겟 메소드의 명령이 실행시 예외가 발생된 후 삽입되어 실행될 명령을 작성한 메소드 - After Throwing Method*/
	public void afterThrowingLog() {
		System.out.println("### [after-Throwing] 핵심관심코드 실행시 예외가 발생되면 삽입되어 실행될 횡단관심코드 ###");		
	}
	
	//타겟메소드의 명령 실행 전과 후에 삽입되어 실행될 명령을 작성한 메소드 - Around Advice Method - 가장 많이 사용

...
//-> 일반적으로 DatatSource 인터페이스를 상속받은 자식클래스로 객체 생성
...
public class DataSourceServlet_12 extends HttpServlet {
   ...
   protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      ...
      //BasicDataSource 객체(DataSource 객체) 생성 -> 컨넥션 객체를 미리 여러개 만들어서 저장하기위해서
      ...
      dataSource.setInitialSize(10);//최초 생성될 Connection 객체의 갯수 변경 - 기본값: 20개
      ...
      out.println("<hr>");
      try {
         //dataSource.getConnection(): dataSource객체에 저장된 다수의 Connection 객체 중 하나를 제공받아 반환하는 메소드
         ...
         out.println("<hr>");
         con.close();//사용중인 컨넥션 객체 제거 - dataSource 객체에게 컨넥션 되돌려줌(대기 상태로 변경)
         
         out.println("<h3>Connection 객체 제거 후</h3>");
         out.println("<p>Idle Connection Number="+dataSource.getNumIdle()+"</p>");//대기상태의 컨넥션 갯수
         out.println("<p>Active Connection Number="+dataSource.getNumActive()+"</p>");

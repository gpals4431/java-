...
//사용자로부터 방명록 게시글(변경값)을 입력받기 위한 서블릿 
//=> 글번호를 전달받아 GUEST 테이블에 저장된 방명록 게시글을 검색하여 입력태그의 초기값으로 설정하고 사용자로부터 방명록 게시글을 입력받기 위한 서블릿
@WebServlet("/modifyForm.itwill")
public class GuestUpdateFormServlet extends HttpServlet {
   ...
   protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      ...
      if(request.getParameter("num")==null) {
         response.sendError(HttpServletResponse.SC_BAD_REQUEST);//405 에러 클라이언트에게 전달
         return;
      }
      
      //전달값을 반환받아 저장 - 정수값으로 변환하여 저장
      int num=Integer.parseInt(request.getParameter("num"));
      
      //글 번호를 전달받아 GUEST 테이블에 저장된 해당 글 번호의 방명록 게시글을 검색하여 DTO 객체를 반환하는 DAO 클래스의 메소드 호출
      ...
      out.println("<hr>");
      out.println("<form action='write.itwill' method='post' name='modifyForm'>");
      //방명록 게시글을 구분하기 위한 글번호를 반드시 전달
      out.println("<input type='hidden' name='num' value='"+guest.getNum()+"'>");//(중요!)글 번호 비교
      out.println("<table>");
		out.println("<tr>");

...
//this 키워드 : 메소드 내부에 숨겨져 있는 키워드로 메소드를 호출한 객체의 
//메모리 주소(HashCode)를 자동으로 전달받아 저장하는 키워드
// => 메소드 내부에 명령에서 객체의 필드 또는 메소드를 사용하기 위한 this 키워드 사용
// => 메소드 내부에서 객체를 대신 표현하기 위해 사용하는 키워드
// => this 키워드를 사용하지 않아도 자동으로 객체의 필드 또는 메소드에 접근하여 사용

//메소드에 this 키워드가 존재해야되는 이유
// => 필드는 객체마다 메모리(Heap Area)에 독립적으로 생성되지만 메소드는 객체와 상관없이
//메모리(MethodArea)에 하나만 생성 - 프로토타입 클래스(Prototype Class)
// => 메소드에서 필드를 사용할 때 this 키워드로 객체를 명확히 구분하여 필드를 접근하도록 사용
...
public class Member {
	...
	public Member() {
		...
		this("NoID","NoName","NoEmail"); // >> 생성자에서 다른 생성자를 호출할때 사용, 이런 경우는 무조건 첫번쨰 명령이어야함
	}
	...
	public Member(String id, String name) {
		super();
		this.id = id;//this : '내가 가지고 있는' 으로 해석
		this.name = name;
	}

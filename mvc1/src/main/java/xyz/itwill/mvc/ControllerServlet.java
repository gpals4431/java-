...
//=> @WebServlet("*.do") : 클라이언트가 [XXX.do]확장자 형식의 URL 주소를 요청한 경우 웹프로그램(서블릿) 실행
//=> @WebServlet 어노테이션 대신 [web.xml] 파일의 엘리먼트를 사용하여 클래스를 웹프로그램으로
//등록하고 URL 매핑 처리 가능 -> [web.xml]사용하는 것이 유지보수가 더 편해서 권장됨
//@WebServlet("*.do")
public class ControllerServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		String reqStringURI = request.getRequestURI();
		...
		String command = reqStringURI.substring(contextPath.length());///XXX.do만 추출
		...
		// => 요청 처리 메소드가 선언된 모델 역활의 클래스 작성 - 모델 역활의 클래스는 인터페이스를 상속받아 작성 : 동일한 규칙으로 처리하기 위해서
		...
		if(actionForward.isForward()) {//ActionForward 객체의 forward 필드값이 [true]인 경우
			...
			request.getRequestDispatcher(actionForward.getPath()).forward(request, response);//스레드 이동

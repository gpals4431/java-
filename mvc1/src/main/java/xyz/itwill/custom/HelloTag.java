...
//1. 태그 속성과 태그 내용이 없는 커스텀 태그의 클래스
public class HelloTag extends TagSupport {// 커스텀 태그 최초 사용시 WAS가 클래스를 객체로 만들어줌
	...
	// JSP 문서에서 커스텀 태그의 시작태그를 사용할때마다 호출되는 메소드
	@Override
	public int doStartTag() throws JspException {
		//System.out.println("HelloTag 클래스의 doStartTag() 호출");
		
		try {
		//부모클래스부터 제공받은 PageContext 객체(PageContext 변수에 저장)를 이용하여 웹프로그램 작성에 필요한 객체를 반환받아 명령 작성
			//pageContext.getOut() : JSP Writer 객체 반환 - 응답 문서를 생성하기 위한 출력스트림을 반환하는 매소드
	...
	// JSP 문서에서 커스텀 태그의 태그내용을 사용할때마다 자동 호출되는 메소드
	@Override
	public int doAfterBody() throws JspException {
		// System.out.println("HelloTag 클래스의 doAfterBody() 호출");
		...
		//EVAL_BODY_AGAIN : 태그내용을 클라이언트에게 다시 전달할 경우 사용하는 상수 - 일종의 반복처리
		return SKIP_BODY;
	}

	// JSP 문서에서 커스텀 태그의 종료태그를 사용할때마다 호출되는 메소드
	@Override
	public int doEndTag() throws JspException {
		// System.out.println("HelloTag 클래스의 doEndTag() 호출");
		...
		return SKIP_PAGE;//세 번 호출해도 <DIV>태그 한번만 출력 됨 
	}
}

...
<body>
	<h1>Formatter - 날짜와 시간변환 태그</h1>
	<hr>
	<!-- EL은 출력만 객체를 만들지는 못함 -> 객체 만들때는 스클립틀릿 사용
	서버 시스템의 현재 날짜와 시간이 저장된 Date 객체를 생성하여 스코프 객체의 속성값으로 저장 -->
	<c:set var="now" value="<%=new Date() %>"></c:set>
	<!-- EL를 이용하여 Date 객체를 제공받아 출력 처리할 경우 toString()메소드 자동 호출 -->
	<p>now=${now }</p>
	
	<!-- formateDate 태그 : Date 객체에 저장된 날짜와 시간을 원하는 패턴의 문자열로 변환하여 제공하는 태그
	...
	=> dateStyle 속성값이 [short]인 경우 [yy. M. d] 패턴의 문자열로 변환되어 제공
	 -->
	<p>now(날짜) = <fmt:formatDate value="${now }" type="date" dateStyle="full"/> <!--2023년 7월 14일  -->
	<p>now(날짜) = <fmt:formatDate value="${now }" type="date" dateStyle="short"/> <!--23.7.14  -->
	
	<!-- type 속성값을 [time]로 설정한 경우 기본적으로 [a h:mm:ss] 패턴의 문자열로 변환되어 제공
	=> dateStyle 속성값이 [full]인 경우 [a h시 mm분 ss초 z] 패턴의 문자열로 변환되어 제공
	=> dateStyle 속성값이 [short]인 경우 [a h:mm] 패턴의 문자열로 변환되어 제공 -->
	<p>now(시간) = <fmt:formatDate value="${now }" type="time" dateStyle="full"/>
	<p>now(시간) = <fmt:formatDate value="${now }" type="time" dateStyle="short"/>
	<%-- type 속성 : date(날짜), time(시간), both(날짜와 시간) 중 하나를 속성값으로 설정 --%>

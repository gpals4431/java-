...
<body>
	<h1>Core - URL 관리 태그</h1>
   	...
   	문제점) MVC 디자인 패턴을 이용한 JSP Model-2 방식의 웹프로그램 작성시 요청 웹프로그램(컨트롤러)의
   	경로와 응답 웹프로그램(뷰)의 경로가 다른 경우 404에러 발생
   	해결법) 웹자원의 경로를 절대경로로 표현하여 제공 -->
   	<img src="images/Koala.jpg" width="200">
   	
   	<%--웹자원의 경로를 절대경로로 표현하여 제공 
   	절대경로 : 최상위 디렉토리를 기준으로 웹자원의 경로를 표현하는 방법
   	문제점) 컨텍스트의 이름이 변경될 경우 컨텍스트 경로가 변경될 수 있음 -> 404 에러 발생
   	해결법) 컨텍스트 경로를 제공받아 절대경로로 표현 --%>
   	<img src="/mvc1/jstl/images/Koala.jpg" width="200">
   	...
   	value 속성 : 컨텍스트 경로를 제외한 웹자원의 절대 경로를 속성값으로 설정 -->

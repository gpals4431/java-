...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!--
taglib 디렉티브를 사용하여 JSTL 의 CORE 태그 라이브러리를 JSP 문서에 포함해야만
...
<body>
	...
	<hr>
<%--
 set 태그 : 스코프 객체의 속성값으로 객체를 저장(변경)하기 위한 태그 - setAttribute와 동일한 역할
var 속성 : 스코프 객체에 저장될 속성값을 구분하기 위한 속성명을 태그 속성값으로 설정
value 속성 : 스코프 객체에 저장될 속성값(객체)을 태그 속성값으로 설정 - 10은 integer 객체, 정수값x
scope 속성 : 스코프 객체에 저장된 속성값의 사용범위를 태그 속성값으로 설정 
=> page, request, session, application 중 하나를 속성값으로 설정
=> 스코프 속성 생략시 [page]속성값을 기본값으로 사용
 --%>
	<c:set var="su" value="10" scope="page"/>
	<p>su=${su }</p>
	
	<!-- value 속성 대신 태그 내용을 사용하여 스코프 객체에 저장될 속성값 설정 가능
	스코프 객체의 속성명이 같은 경우 스코프 객체의 속성값 변경 처리  -->
	<c:set var="su">20</c:set>
	<p>su=${su }</p>
		
	<!-- set 태그의 value속성값으로 EL를 이용하여 값을 제공받아 속성값으로 저장 가능
	...
	▶value : 필드값을 저장될 객체를 태그 속성값으로 설정 -->

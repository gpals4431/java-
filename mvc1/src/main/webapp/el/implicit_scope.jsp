...
<%
	...
	// => 속성값의 사용범위 : Page Scope, Request Scope, Session Scope, Application Scope
	pageContext.setAttribute("pageName", "홍길동");//Page Scope
	request.setAttribute("requestName", "임꺽정");//Request Scope
	session.setAttribute("sessionName", "전우치");//Session Scope = 클라이언트의 갯수만큼 존재
	...
	pageContext.setAttribute("name", "홍경래");//Page Scope - 기존(=홍길동) 대신 새로운 속성값(=홍경래)으로 변경
...
<body>
	...
	<ul>
		<%-- EL 표현식으로 속성명을 사용하여 속성값을 제공받기 위한 순서 --%>
		...
		<%-- => EL 내장객체 : pageScope, requestScope, sessionScope, applicationScope - 전부 Map 객체
		속성명 : Key 

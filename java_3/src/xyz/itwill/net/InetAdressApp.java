...
/*		InetAddress 클래스: 네트워크 식별자(ip주소 및 호스트 이름)를 저장하기 위한 클래스 
 		InetAddress getLocalHost: 현재 사용중인 컴퓨터의 네트워크 식별자자 저장된 InetAddress 객체를 반환하는 메소드  
 		>> UnknownHostException 발생( 일반 예외 ) : 네트워크에서 사용하는 호스트 이름의 컴퓨터를 검색할 수 없는 경우 발생되는 예외
 
  		현재 사용중인 컴퓨터는 기본적으로 [127.0.0.1]의 IP 주소가 제공되며 [localhost]라는 이름으로 호스트이름 설정 > 컴퓨터 내부에서만 사용
 ...
 */
		...
		// > 호스트 네임은 임의로 저장, 아이피 주소를 알고있는 것을 권장함
		System.out.println("myComputer ="+ myComputer);
		
		//InetAddress. getHostName() : InetAddress 객체에 저장된 네트워크 식별자에서 호스트 이름을 문자열로 변환하여 반환하는 메소드
		System.out.println("myComputer ="+ myComputer.getHostName());
		//InetAddress. getHostAddress() : InetAddress 객체에 저장된 네트워크 식별자에서 IP주소를 문자열로 변환하여 반환하는 메소드
		...
		for(InetAddress address:naver) {
			...
			System.out.println("www.naver.com의 IP주소 ="+ address);
		}
		System.out.println("===========================================================");

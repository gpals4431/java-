...
public class MessageReceiveApp {
	public static void main(String[] args) throws IOException{
		...
		DatagramSocket socket = new DatagramSocket(4000);//1-1
		
		//패킷에 의해 전달된 값(메세지)을 저장하기 위한 byte 배열 선언
		byte[] data = new byte[1024];//메세지를 이만큼 저장할거다//1-2
		...
		//>> 받을떄는 받아서 저장할것만 필요 주소나 포트 필요x
		DatagramPacket packet = new DatagramPacket(data, data.length);//6
		
		System.out.println("메세지 수신 중 ... ");//7
		...
		String message = new String(data);//8
		
		System.out.println("[결과] 메세지 >> " +message);//9

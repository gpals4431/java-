...
public class MapApp {
	public static void main(String[] args) {
		//이름(고유값)과 객체에 대한 제네릭타입 2개에 자료형을 전달하여 객체를 생성
		Map<String, String> map = new  HashMap<String, String>();
		
		//Map.put(K,V) : map 객체에 엔트리를 추가하는 메소드
		// => Map 객체에 저장되는 엔트리는 이름(key)을 Set 객체로 저장 - key는 중복될 수 없다.(Set 특징)
		map.put("1000", "홍길동");
		map.put("2000", "임꺽정");
		map.put("3000", "전우치");
		map.put("4000", "일지매");
		...
		//Map 객체에 저장된 모든 엔트리를 문자열로 변환하여 반환하는 메소드
		System.out.println("map.toString"+map.toString());
		System.out.println("==============================================================================");
		//★Map.put(K,V) : 메소드 호출시 이름이 중복된 경우 해당 이름의 엔트리의
		//객체(value)를 변경하여 저장
		map.put("2000", "임걱정");
		System.out.println("map.toString"+map.toString());
		System.out.println("==============================================================================");
		map.remove("4000");//Map 객체에 저장된 엔트리에서 매개변수로 전달받은 이름에 대한 엔트리를 검색하여 삭제하는 메소드
		System.out.println("map"+map);
		System.out.println("==============================================================================");
		//★map.get(K key): Map 객체에 저장된 엔트리에서 매개변수로 전달받은 이름에 대한 엔트리를 검색하여 엔트리의 객체를 반환하는 메소드

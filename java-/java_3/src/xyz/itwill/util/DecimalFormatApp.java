...
//DecimalFormat 클래스 : Number 객체(숫자값)와 String객체를 서로 반대로 변환하기 위한 기능을 메소드로 제공하기 위한 클래스
public class DecimalFormatApp {
	public static void main(String[] args) {
		...
		// >> 패턴문자 : #(한개의 숫자 -공백), 0(숫자 -0), 콤마, 소숫점, 달러
		...
		//decimalFormat.getInstance().format(money) : 기본 패턴정보가 저장된 DecimalFormat객체를 반환하는 메소드
		// >> 3자리마다 , 를 사용하여 숫자값을 표현하는 패턴정보
		System.out.println("금액 = "+decimalFormat.getInstance().format(money)+"원");
		//decimalFormat.getCurrencyInstance().format(money) : 기본 패턴정보가 저장된 DecimalFormat객체를 반환하는 메소드

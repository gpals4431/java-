...
// => 형식에 맞지않는 값이 입력될 경우 에러 메세지 출력 후 프로그램 종료
public class RegularExpressionApp {
	public static void main(String[] args) {
		...
		//Pattern클래스 : 정규표현식을 사용하기 위한 기능의 메소드를 제공하는 클래스
		//Pattern.matches(String regEx, CharSequence input) : 정규표현식과 입력값을 비교하여 정규표현식에 
		//패턴과 입력값의 패턴이 다르면 [false]를 반환하고 같으면 [true]를 반환하는 메소드
		...
		*/
		...
		//-는 가운데에 넣으면 범위가돼서 -는 마지막에 작성 & ^맨앞 = not
		...
		*/
		...
		String emailReg="^([a-zA-z0-9_-]+@[a-zA-z0-9_-]+(\\.[a-zA-z0-9_-]+)+)*$";//\.자바에서쓰는 회피문자, \\.정규표현 회피문자

...
public class CalculatorFrameApp extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		// >> 오브젝트 참조변수를 통해 오브젝트 클래스밖에 호출 못함,
		// 이벤트가 발생된 컴퍼넌트(JButton)를 반환받아 저장
		// >> 명시적 객체형변환 사용하여 Object 타입의 객체를 JButton 객체로 변환하여 저장
		JButton eventButton = (JButton)e.getSource();
		
		//이벤트가 발생된 JButton 컴퍼넌트를 비교하여 명령을 선택 실행
		if(eventButton == b_C ) {//이벤트가 발생된 버튼이 [C]인 경우 
			operation="";//연산식을 저장하기위한 필드 초기화
			
			label.setText("0");//JLabel컴퍼넌트 초기화
		}else if (eventButton == b_equals) {//이벤트가 발생된 버튼이 [=]인 경우 
			//연산식에서 검색하기 위한 연산자가 저장된 배열 선언
			String[] operatorArray = {"*","/","+","-"};
			
			int index=-1;//연산식의 위치값(첨자)를 저장하기 위한 변수
			//연산식에서 연산자를 검색하기위한 반복문 
			for(int i=0 ; i<operatorArray.length;i++) {
				index = operation.lastIndexOf(operatorArray[i]);
				if(index!=-1)break;
			}
				//연산식에서 연산자를  찾을 수 없는 경우 이벤트 처리메소드 종료 
				if(index<=0)return;
				...
				try {
					...
					//label.setText(result+"");//""없으면 에러 setText는 문자열이기떄문에 result숫자값오면 안됨
				 
					operation="";
				}catch(ArithmeticException exception) {
					operation="";
					label.setText("0으로 나눌수없습니다");
				}catch(NumberFormatException exception) {
					//JOptionPane 클래스 : 다이얼로그를 제공하기 위한 클래스
					//JOptionPane.showMessageDialog(Component parent, String message)
					// >> 메세지 다이얼로그를 출력하는 메소드
					JOptionPane.showMessageDialog(this, "입력한 연삭식이 형식에 맞지않음");
					label.setText("0");
					operation="";
		...
		}else {//이벤트가 발생된 버튼이 나머지인 경우 
			//버튼의 문자열을 반환받아 operation필드에 추가하여 저장
			//JButton.getText() : 버튼의 문자열(라벨명)을 반환하는 메소드
			operation+=eventButton.getText();
			
			//operation 필드에 저장된 문자열을 사용하여 JLabel 컴퍼넌의 문자열을 변경

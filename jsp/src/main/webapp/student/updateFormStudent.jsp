...
<h1 align="center">학생정보 변경</h1>
	...
	hidden 타입으로 전달하면 학생번호 입력창이 아예 숨겨지니까 readonly로 학생번호 입력창은
	보여주고 변경만 하지 못하도록 설정--%>
	...
	function submitCheck() {
		...
		<%--
		"location.href='updateFormStudent.jsp?no=<%=student.getNo()%>' 
		getParameter 입력 받은 문자 값을 반환하는 메소드 -> 질의문자열로 안보내는 이유?
		form 태그로 url에 getparameter로 값을 보내면 나머지 name, phone 등을 하나하나 부호화
		처리해줘야해서 input 태그로 no 값을 같이 보내줌
		--%>
		studentForm.submit();
	} 

...
<%}else{//답글인 경우%>
	<h1>답글쓰기</h1>
<%}%>
<!-- 파일을 입력받아 전달하기 위해 반드시 enctype 속성값을 "multipart/form-date로 설정  -->
<form action="<%= request.getContextPath()%>/review/review_write_action.jps" method="post"
	 enctype="multipart/form-date" id="reviewForm">
<!-- 리뷰에 파일을 입력받기 위해서 enctype="multipart/form-date로 저장 -->

	<!-- action.jsp가 필요로 하는 value값 전달한걸 또 전달해야함 ,  새글일때는 0,0,0,1 저장 답글은 pagENum저장-->
	<input type="hidden" name="ref" value="<%=ref%>">
	<input type="hidden" name="restep" value="<%=restep%>">
	<input type="hidden" name="relevel" value="<%=relevel%>">
	<input type="hidden" name="pageNum" value="<%=pageNum%>">
	<table>
		<tr>
			...
			<td>
				<input type="text" name="subject" id="subject" size="40">
				<input type="checkbox" name="secret" id="secret" size="2">비밀글
			</td>
		</tr>
		<tr>
			<th>내용</th>
		...
		<tr>
			<th>리뷰이미지</th>
			<td>
				<input type="file" name="reviewimg"><!-- id는 입력값 검증을 위해 사용, 검증안해도되면 id 필요x -->
			</td>
		</tr>
		<tr>
			<th colspan="2">

...
<%-- 글번호를 전달받아 REVIEW테이블에 저장된 게시글을 검색하여 입력태그의 초기값으로 설정하고
사용자로부터 게시글 변경값을 입력받기 위한 JSP 문서
=> 로그인 상태의 사용자 중 게시글 작성자이거나 관리자인 경우에만 요청 가능한 JSP 문서
=> [글변경] 태그를 클릭한 경우 [review/remove_modify_action]문서 요청 - 입력값(게시글 )전달 --%>
...
<%
	...
	//게시글 작성자 또는 관리자가 아닌 경우 - 비정상적인 요청
...
<form
	action="<%=request.getContextPath()%>/review/review_modify_action.jsp"
	...
	<table>
		<tr>
			...
			<td><input type="text" name="subject" id="subject" size="40"
				value="<%=review.getSubject()%>"> <input type="checkbox"
				name="secret" value="2" <%if(review.getStatus()==2) { %>
				checked="checked" <% } %>>비밀글</td>
		<tr>
			<th>내용</th>
			<td><textarea rows="7" cols="60" name="content" id="rContent"><%=review.getContent() %></textarea>
			</td>
		</tr>
		<tr>
			<th>리뷰이미지</th>
			<td><input type="file" name="reviewimg">
				<div style="color: red;">이미지를 변경할 경우에만 파일을 입력해 주세요.</div> <% if(review.getReviewimg()!=null) { %>
				<img
...
<div id="mesage" style="color: red;"></div>

<script type="text/javascript">
$("#subject").focus();

$("#reviewForm").submit(function() {
	if($("#message").text("제목을 입력해주세요");
		($("#subject").focus();
		return false;
	}
	
	if($("#rContent").val()==""){
		if($("#message").text("내용을 입력해주세요");
		($("#rContent").focus();
		return false;
	}
});

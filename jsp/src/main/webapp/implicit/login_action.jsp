...
<%
	...
	if(request.getMethod().equals("GET")) {//JSP 문서를 GET 방식으로 요청한 경우 - 비정상적인 요청
		...
		*/
		...
		response.sendRedirect("login_form.jsp?message="+message); ?: 질의문자열
		return;
		*/
		
		//바인딩된 세션에 에러메세지 저장
		//session.setAttribute(이름, 값)): 이름에 값을 저장하는 메소드 
		session.setAttribute("message", "비정상적인 방법으로 페이지를 요청 하였습니다.");
		response.sendRedirect("login_form.jsp");//리다이렉트 이동, 해당 주소로 응답
		return;
	}

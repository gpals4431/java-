...
public class MemberDAO extends JdbcDAO {
	...
	//회원정보를 전달받아 member테이블에 삽입하고 삽입행의 갯수를 반환하는 메소드
	public int insertMember(MemberDTO member) {
		...
		try {
			...
			System.out.println("[에러]insertMember()메소드의 SQL 오류 발생"+e.getMessage());
		}finally {
			close(con,pstmt);
		}
		return rows;
	...
	public MemberDTO selectMember(String id){
		...
		try {
			...
			if(rs.next()) {
				...
				System.out.println("[에러]selectMember()메소드의 SQL 오류 발생"+e.getMessage());
			}finally {
				close(con,pstmt,rs);
			}
			return member;

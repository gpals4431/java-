...
@Service //dao 객체 여러개 받아서 데이터 처리해줌
@RequiredArgsConstructor//생성자 레벨을 사용하면 @autowired로 필드마다 주입해줄 필요없이 한번만 작성해줘도됨
public class PointBoardServiceImpl implements PointBoardService {
	...
	//매개변수로 게시글을 전달받아 pointboard 테이블에 게시글로 삽입하고 작성자에 대한 회원정보를
	//pointuser 테이블에서 검색하여 반환하는 메소드
	//=> pointuser 테이블에서 게시글 작성자의 회원정보의 포인트가 증가되도록 변경 처리

...
public class ProtectedAnimalsDAOImpl extends JdbcDAO implements ProtectedAnimalsDAO{
   ...
   @Override
   public ProtectedAnimalsDTO selectRegNoAnimalsList(String reg_no) {
      Connection con=null;
      PreparedStatement pstmt=null;
      ResultSet rs=null;
      ...
      try {
         ...
         if(rs.next()) {
            ...
            animal.setAdopt(rs.getString(rs.getString("ADOPT")));
            animal.setReg_no(rs.getString("REG_NO"));
         }
         
      } catch (SQLException e) {
         System.out.println("[에러]selectRegNoAnimalsList() 메소드의 SQL 오류 = "+e.getMessage());
      }finally {
         close(con, pstmt, rs);
      }
      return animal;
